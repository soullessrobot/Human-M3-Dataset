...
class JointsDataset(Dataset):
    ...
    def compute_human_scale(self, pose, joints_vis):
        ...
        return np.clip(np.maximum(maxy - miny, maxx - minx)**2,  1.0 / 1024**2, 1024**2)
    ...
    def generate_target_heatmap(self, joints, joints_vis):
        ...
        if self.target_type == 'gaussian':
            ...
            for n in range(nposes):
                ...
                cur_sigma = self.sigma * np.sqrt((human_scale / (1024.0 * 1024.0)))
    ...
    def generate_3d_target(self, joints_3d, cur_sigma = None):
    ...
    def generate_input_heatmap(self, joints):
        ...
        show = False
        if show:
            # target_show = np.mean(target, axis= 0)
            ...
            for i in range(target.shape[0]):
                if i > 7:
                    continue
                ax = fig.add_subplot(eval('33'+str(i+1)))
                ax.imshow(target[i])
            ax = fig.add_subplot(eval('33'+str(9)))
            ax.imshow(np.mean(target, axis = 0))
            # for jointer in joints:
            #     ax.scatter(jointer[:,0]/ feat_stride[0], jointer[:,1]// feat_stride[1], marker = '*', c = 'r', s = 1)
            plt.show()
        return target
