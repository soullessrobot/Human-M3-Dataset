...
def create_logger(cfg, cfg_name, phase='train'):
    ...
    #     open(final_log_file, mode = 'w')
    head = '%(asctime)-15s %(message)s'
    # logging.basicConfig(filename=str(final_log_file), format=head, level = logging.DEBUG)
    # logger = logging.getLogger()
    # logger.setLevel(logging.INFO)
    logger = logging.getLogger() #str(final_log_file)
    logger.addHandler(logging.FileHandler(str(final_log_file)))
    logger.setLevel(logging.INFO)
